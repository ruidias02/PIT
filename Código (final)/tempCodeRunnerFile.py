...
def remover_utilizador():
    ...
    if not user_id:
        return "ID do utilizador não fornecido.", 400
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"Erro ao remover utilizador: {response.text}", 500
    except requests.RequestException as e:
        return f"Erro de conexão: {str(e)}", 500
    ...
    if request.method == 'POST':
        ...
        if 'image' in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                # Salvar com nome único baseado no user_id
                ...
                # Enviar para o gestor de serviço fazer a atualização no banco de dados
                try:
                    response = requests.post(
                    ...
                    image_path = rel_path
                    # Limpar as áreas salvas quando uma nova imagem é carregada
                    response = requests.post(
        ...
        # Verificar se é adição de uma área
            ...
            try:
                # Se tiver area_id, é uma edição
                if area_id and area_id.strip():
                    response = requests.post(
                        "http://127.0.0.1:5000/update_user_area",
                        json={
                            "area_id": int(area_id),
                            "user_id": user_id,
                            "x": float(x),
                    ...
                    # Caso contrário, é uma nova área
                    response = requests.post(
                ...
                if response.status_code != 200:
                    return f"Erro ao {'atualizar' if area_id else 'salvar'} área: {response.text}", 500
                    
                # Atualizar a lista de áreas
                query = """
                    SELECT a.id, a.x, a.y, a.sensor_x, a.sensor_y, a.width, a.height, a.description, a.sensor_id, 
                          IFNULL(s.status, 'Sem sensor') as sensor_status
                    FROM areas_utilizador a
                    LEFT JOIN sistemas_sensores s ON a.sensor_id = s.id
                    WHERE a.user_id = %s 
                    ORDER BY a.id
        ...
        # Verificar se é exclusão de área
            ...
            try:
                ...
                # Atualizar a lista de áreas
                query = """
                    SELECT a.id, a.x, a.y, a.width, a.height, a.description, a.sensor_id, 
                          IFNULL(s.status, 'Sem sensor') as sensor_status
                    FROM areas_utilizador a
                    LEFT JOIN sistemas_sensores s ON a.sensor_id = s.id
                    WHERE a.user_id = %s 
...
def area_sensor_data(area_id):
    ...
    # Buscar os dados mais recentes do sensor
    query = """
    ...
    # Buscar alertas recentes para este sensor
    query = """
...
def area_risk_heatmap(area_id):
    ...
    # Buscar todas as áreas com sensores nesta imagem
    query = """
        SELECT a.id, a.description, a.sensor_id, a.x, a.y, a.width, a.height,
    ...
    for sensor_area in areas_with_sensors:
        ...
        query = """
            ...
            LIMIT 100
        """
        sensor_data = execute_query(query, (sensor_area['sensor_id'],))
        
        if not sensor_data:
            continue
            
        # Calcular valores médios
        avg_temp = sum(data['temperature'] for data in sensor_data) / len(sensor_data)
        avg_humidity = sum(data['humidity'] for data in sensor_data) / len(sensor_data)
        avg_co2 = sum(data['CO2'] for data in sensor_data) / len(sensor_data)
        avg_lum = sum(data['Lum'] for data in sensor_data) / len(sensor_data)
        
        # Calcular o risco
        risk_value = calculate_risk(avg_temp, avg_humidity, avg_co2, avg_lum)
    ...
    return render_template('risk_heatmap.html',
                          ...
                          type='risk')  # Adicionar o parâmetro type)
...
def api_sensor_data(area_id):
    ...
    # Verificar se a área pertence ao usuário e obter o caminho da imagem
    query = """
        SELECT i.image_path
        FROM areas_utilizador a
        JOIN utilizador_imagem i ON i.user_id = a.user_id
    ...
    sensors_data = []
    for sensor_area in areas_with_sensors:
        # Buscar APENAS O ÚLTIMO REGISTRO do sensor
        query = """
        ...
        if not last_reading:
            continue  # Pular se não houver leituras
        
        # Calcular o risco com o ÚLTIMO VALOR registrado
...
def api_get_sensor_co2_data(area_id):
    ...
    for sensor_area in areas_with_sensors:
        ...
        # Calcular valores médios dos últimos dados para reduzir flutuações
        avg_co2 = sum(data['CO2'] for data in sensor_data) / len(sensor_data)
        avg_temp = sum(data['temperature'] for data in sensor_data) / len(sensor_data)
        avg_humidity = sum(data['humidity'] for data in sensor_data) / len(sensor_data)
        avg_lum = sum(data['Lum'] for data in sensor_data) / len(sensor_data)
...
def area_humidity_heatmap(area_id):
    ...
    LOW_HUM_THRESHOLD = 30  # Humidade baixa é problema
    ...
    for sensor_area in areas_with_sensors:
        ...
        # Calcular valores médios para humidade
        avg_humidity = sum(data['humidity'] for data in sensor_data) / len(sensor_data)
        # Também podemos incluir a média de temperatura para referência
        avg_temp = sum(data['temperature'] for data in sensor_data) / len(sensor_data)
...
# API endpoint para atualização dos dados de humidade
...
def get_humidity_data(area_id):
    ...
    # Preparar dados de sensores para retornar como JSON
    sensors_data = []
    for sensor_area in areas_with_sensors:
        # Buscar os dados mais recentes do sensor
        query = """
...
def get_data_sensor(area_id):
    ...
    # Verificar se a área pertence ao usuário - adicionar cache se possível
    query = """
    ...
    # Esta consulta pode ser otimizada se necessário para reduzir carga no DB
    query = """
        SELECT a.id, a.description, a.sensor_id, a.x, a.y, a.width, a.height,
               a.sensor_x, a.sensor_y
        FROM areas_utilizador a
    ...
    for sensor_area in areas_with_sensors:
        ...
        # Importante: esta consulta será executada frequentemente (a cada segundo)
        query = """
    ...
    # Opcional: adicionar cache-control headers para evitar caching no navegador
